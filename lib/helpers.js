...
SchemaContext.prototype.resolvePath = function resolve (target) {
  target = (typeof target === 'string') ? target : target.$ref;
...
SchemaContext.prototype.resolve = function resolve (target) {
  ...
  if (this.schemas[target]) {
    return this.schemas[target];
  }

  // Root schema
  var parsed = uri.parse(target);
  var fragment = parsed && parsed.hash;
  if (fragment === '#') {
    return this.schemas[this.base];
  }

  // Else try walking the property pointer
  var document = fragment && fragment.length && target.substr(0, target.length - fragment.length);
  if (document && this.schemas[document]) {
    var subschema = exports.objectGetPath(this.schemas[document], fragment.substr(1));
    if (subschema === undefined){
      throw new SchemaError("no such schema " + fragment + " located in <" + document + ">", this.schema);
    }
    return subschema;
  }

  throw new SchemaError("no such schema <" + target + ">", this.schema);
};
...
/**
 * Determines if the provided value is a pointer to a schema.
 * @param p
 * @return boolean
 */
exports.isPointer = function isPointer(p) {
